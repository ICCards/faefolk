...
var input_received = true #boolean to communicate between threads if new inputs have been received
 #encloses input_received and also the game variable that tracks current game status
...
#---functions---
func thr_network_inputs(data): #thread function to process data from network
	var result = null
	var packet_idx = 1
	var new_input = false
...
#			return
	...
	result = data #receive a single packet
	if result:
...
func _physics_process(_delta):
	if input_arrival_array[frame_num]:
		...
		for _x in range(packet_amount):
			Server.rpc_id(1,"input",[1, frame_num, (frame_num + input_delay)%256])  #send request for needed input
