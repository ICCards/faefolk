...
func initialize_slots():
	...
	for i in range(slots.size()):
		...
		if PlayerData.player_data["campfires"][get_parent().id].has(str(i)):
			slots[i].initialize_item(PlayerData.player_data["campfires"][get_parent().id][str(i)][0], PlayerData.player_data["campfires"][get_parent().id][str(i)][1], PlayerData.player_data["campfires"][get_parent().id][str(i)][2])
...
func able_to_put_into_slot(slot):
	...
	if slot.slot_index == 0: # fuel
		return holding_item_name == "wood" or holding_item_name == "coal"
	elif slot.slot_index == 1: # ingredients
	...
	elif slot.slot_index == 2 or slot.slot_index == 3: # yield
		return false
	return true
