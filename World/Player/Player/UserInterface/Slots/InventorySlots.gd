	...
	if event is InputEventMouseButton:
		if event.button_index == MOUSE_BUTTON_LEFT && event.pressed:
			if find_parent("UserInterface").holding_item != null:
				...
				if get_parent().name == "Chest" or get_parent().name == "Tool cabinet" or get_parent().name == "Crate":
...
func left_click_not_holding_chest(slot):
	for i in range(get_node("../ChestSlots").get_children().size()):
		if PlayerData.player_data["ui_slots"][get_parent().id].has(str(i)):
			if PlayerData.player_data["ui_slots"][get_parent().id][str(i)][0] == slot.item.item_name and not PlayerData.player_data["ui_slots"][get_parent().id][str(i)][1] == int(JsonData.item_data[slot.item.item_name]["StackSize"]):
				auto_add_to_chest_slot(slot,i)
				return
	for i in range(get_node("../ChestSlots").get_children().size()):
		if not PlayerData.player_data["ui_slots"][get_parent().id].has(str(i)):
...
func auto_add_to_chest_slot(slot_clicked,i):
	...
	var able_to_add = stack_size - PlayerData.player_data["ui_slots"][get_parent().id][str(i)][1]
