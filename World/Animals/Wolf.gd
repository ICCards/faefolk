...
func move(_velocity: Vector2) -> void:
	if not visible or $EnemyTornadoState.tornado_node or stunned or destroyed or attacking or state == IDLE:
		return
	if frozen:
...
func _physics_process(delta):
	...
	if (player.state == 5 or player.get_node("Magic").invisibility_active) and chasing:
		...
		start_chase_state()
	if chasing and (position+Vector2(0,-9)).distance_to(player.position) < 30:
		state = ATTACK
		attack()
...
func _on_HurtBox_area_entered(area):
	...
	if area.special_ability == "fire":
		var randomPos = Vector2(randf_range(-4,4), randf_range(-6,0))
...
func end_chase_state():
	chasing = false
	navigation_agent.set_deferred("max_speed", 60)
