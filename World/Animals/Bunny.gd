...
func _physics_process(delta):
	...
	bunny_sprite.flip_h = _get_direction_string(sync_velocity) != "Right"
...
#
#
func _get_direction_string(velo) -> String:
	if velo.x > 0:
		return "Right"
	return "Left"
#
...
func _on_HurtBox_area_entered(area):
	get_parent().rpc_id(1,"mob_hit",{"player_id":Server.player_node.name,"id":name, "t":area.tool_name})
#	sound_effects.set_deferred("stream", load("res://Assets/Sound/Sound effects/animals/bunny/hurt"+str(rng.randi_range(1,4))+".mp3"))
#	sound_effects.set_deferred("volume_db", Sounds.return_adjusted_sound_db("sound", 0))
#	sound_effects.call_deferred("play")
