...
func move(_velocity: Vector2) -> void:
	if $EnemyTornadoState.tornado_node or stunned or destroyed or state == IDLE:
		return
	elif frozen:
...
func _physics_process(delta):
	...
	if not start_jump and state == CHASE:
		...
		if (position+Vector2(0,-9)).distance_to(Server.player_node.position) < 90 and not jump_delay and state != IDLE:
			jump_forward()
...
func jump_forward():
	...
	if not cancel_jump and not destroyed:
		...
		await get_tree().create_timer(0.35).timeout #slime_sprite.animation_finished
...
func _on_HurtBox_area_entered(area):
	...
	if area.special_ability == "fire":
		var randomPos = Vector2(randf_range(-4,4), randf_range(-4,4))
