...
func add_boundary_tiles():
	for amt in range(10):
		for x in range(map_w+20):
			walls.set_cell(x-10,-amt,0)
			walls.set_cell(x-10,map_h+amt,0)
		for y in range(map_h):
			walls.set_cell(-amt-1,y,0)
			walls.set_cell(map_w+amt,y,0)
	for x in range(map_w):
		for y in range(map_h):
...
func dig_caves():
	randomize()
	for i in range(iterations): # Pick a random point with a 1-tile buffer within the map
...
func check_nearby(x, y):
	var count = 0
	if walls.get_cell(x, y-1)   != -1:  count += 1
	if walls.get_cell(x, y+1)   != -1:  count += 1
	if walls.get_cell(x-1, y)   != -1:  count += 1
	if walls.get_cell(x+1, y)   != -1:  count += 1
	if walls.get_cell(x+1, y+1) != -1:  count += 1
	if walls.get_cell(x+1, y-1) != -1:  count += 1
	if walls.get_cell(x-1, y+1) != -1:  count += 1
	if walls.get_cell(x-1, y-1) != -1:  count += 1
	return count
...
func fix_tiles():
	...
	var border_tiles = []
	for loc in walls.get_used_cells():
		if Util.is_border_tile(loc, walls.get_used_cells()):
			border_tiles.append(loc)
	for tile in border_tiles:
		if not Tiles.isValidAutoTile(tile, walls):
			walls.set_cellv(tile,-1)
