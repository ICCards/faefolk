...
func set_random_texture():
	...
	$Starfish.texture = load("res://Assets/Images/Forage/starfish"+ str(type) +".png")
...
func _on_Btn_pressed():
	if $DetectPlayer.get_overlapping_areas().size() >= 1 and Server.player_node.state == 0:
		...
		$Btn.disabled = true
		Input.set_custom_mouse_cursor(preload("res://Assets/mouse cursors/Normal Selects.png"))
		Server.player_node.harvest_forage("starfish"+str(type))
		yield(get_tree().create_timer(0.6), "timeout")
		PlayerInventory.add_item_to_hotbar("starfish", 1, null)
		queue_free()
