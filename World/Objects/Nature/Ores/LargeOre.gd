...
func setTexture():
	...
	if health <= 40:
		...
		Tiles.remove_valid_tiles(Vector2i(location)+Vector2i(-1,0), Vector2(2,2))
...
func hit(data):
	...
	#	MapData.update_object_health("ore_large", name, health)
		if health > 40:
			...
			Tiles.add_valid_tiles(Vector2i(location)+Vector2i(-1,-1), Vector2(2,1))
			large_break = true
...
#		MapData.remove_object("ore_large",name)
		Tiles.add_valid_tiles(Vector2i(location)+Vector2i(-1,0), Vector2(2,1))
		sound_effects.set_deferred("stream", Sounds.ore_break[rng.randi_range(0, 2)])
		sound_effects.set_deferred("volume_db", Sounds.return_adjusted_sound_db("sound", -12))
		sound_effects.call_deferred("play")
