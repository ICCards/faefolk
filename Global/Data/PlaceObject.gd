...
onready var CampfireObject = preload("res://World/Objects/AnimatedObjects/Campfire.tscn")
onready var FirePedestalObject = preload("res://World/Objects/AnimatedObjects/FirePedestal.tscn")
...
func place_object_in_world(id, item_name, location):
	...
	match item_name:
		"torch":
			...
			valid_tiles.set_cellv(location, -1)
		"campfire":
			var campfireObject = CampfireObject.instance()
			campfireObject.name = str(id)
			campfireObject.initialize(location)
			Server.world.call_deferred("add_child", campfireObject, true)
			campfireObject.position = valid_tiles.map_to_world(location) + Vector2(16, 22)
			valid_tiles.set_cellv(location, -1)
		"fire pedestal":
			var firePedestalObject = FirePedestalObject.instance()
			firePedestalObject.name = str(id)
			firePedestalObject.initialize(location)
			Server.world.call_deferred("add_child", firePedestalObject, true)
			firePedestalObject.position = valid_tiles.map_to_world(location) + Vector2(16, 22)
			valid_tiles.set_cellv(location, -1)
		"wood fence":
