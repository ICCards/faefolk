...
var buildings = ["wall", "door", "double door", "door", "idk", "idk"]
var current_item = -1
...
func radial_menu_off():
	...
	hide()
	if current_item != -1:
		get_node("/root/World/Players/" + Server.player_id + "/" + Server.player_id).switch_building_state(buildings[current_item])
