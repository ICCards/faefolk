...
##								get_node("/root/World3D/Players/" + str(result["id"])).Swing(result["d"]["tool"], result["d"]["direction"])
##
##					"ON_HIT":
##						if isLoaded:
##							if player_state == "WORLD":
...
##
##	# Initiate connection to the given URL.
##	var err = _client.connect_to_url(websocket_url,PackedStringArray(), true)
##	if err != OK:
##		print("Unable to connect")
##		set_process(false)
...
##	_client.poll()
#
#@rpc("any_peer") func updateState(data):
#	if world:
#		pass
...
#		latency_array.clear()
#
#@rpc("any_peer") func move(id,position,direction):
#	if id != str(get_tree().get_unique_id()):
#		if world:
#			if world.Players.has_node(id):
#				world.Players.get_node(id).move(position,direction)
#
#@rpc("any_peer") func input(data):
#	if world:
...
#
#
#@rpc("any_peer") func spawn_player(data):
#	player_id = str(get_tree().get_unique_id())
#	print("spawn player called from srever")
#	if str(data.id) == player_id:
#		player = data
#
#@rpc("any_peer") func load_map(value):
#	print("loading map")
#	var key = map.keys()[mapPartsLoaded]
#	print("Loaded " + key)
#	map[key] = value
